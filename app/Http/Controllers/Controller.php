...
class Controller extends BaseController
{
	...
	{
		$this->projects = explode(',', env("GITHUB_PROJECTS", "portfolio"));
		$this->username = env("GITHUB_USERNAME", "member87");
	}
	...
	{
		return in_array($project, $this->projects) || $project == $this->username;
	}
	...
	public function getReadme($project)
	{
		if (!$this->checkProject($project))
			return abort(404);
		//dd('https://raw.githubusercontent.com/' . $this->username . '/' . $project . '/main/README.md');
		return $this->getCache('readme:' . $project, 'https://raw.githubusercontent.com/' . $this->username . '/' . $project . '/HEAD/README.md');
	}
	...
	public function getInfo($project)
	{
		if (!$this->checkProject($project))
			return abort(404);
		return $this->getCache('info:' . $project, 'https://api.github.com/repos/' . $this->username . '/' . $project);
	}
	...
	public function getLanguage($project)
	{
		if (!$this->checkProject($project))
			return abort(404);
		return $this->getCache('language:' . $project, 'https://api.github.com/repos/' . $this->username . '/' . $project . '/languages');
	}
}
